...
module.exports = class Sensor extends Emitter {
  constructor ({ address, baudRate, mockOptions = null }) {
    ...
    this.port.on('close', retry(attempts => new Promise((resolve, reject) => {
      log.warning(`Connection to ${this.uid} lost, reconnection #${attempts}...`)
      this.port.open(error => error ? reject(error) : resolve())
    }), configuration.sensors.reconnection))
