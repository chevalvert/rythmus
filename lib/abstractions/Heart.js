...
module.exports = class Heart extends Sensor {
  constructor (sensorPort) {
    ...
    this.sample = new PulseSample(configuration.sensors.pulseSample)
    ...
    this.on('data', ({ cycles, signal }) => {
      ...
      this.isAlive = cycles > this.calibratedCyclesThreshold
      if (!this.isAlive) {
        this.setLed(false)
      ...
      this.normalizedValue = this.sample.normalize(this.value) || 0
  ...
  get isBeating () { return this.beat.running }
  get hasBeaten () { return this.beat.hasBeaten }
  get confidence () { return this.sample.confidence }
  get median () { return this.sample.median }
  get normalizedMedian () { return this.sample.normalizedMedian }
