...
const hnode = require('hnode')(Object.assign({
...
server.on('newnode', node => {
  node.lockRate(configuration.hnode.lockRate)
...
module.exports = {
  ...
  inside: (index, z, rgb = configuration['colors'].fallback) => {
    const pillar = pillars[index]
    if (pillar) pillar.inside(z, rgb)
  },

  outside: (index, z, rgb = configuration['colors'].fallback) => {
    const pillar = pillars[index]
    if (pillar) pillar.outside(z, rgb)
  },

  set: (index, z, rgb = configuration['colors'].fallback) => {
    const pillar = pillars[index]
...
function getPillarsFromMapping (mapping) {
  ...
  Object.entries(mapping.nodes).forEach(([nodeName, node]) => {
    node.forEach((position, index) => {
      const isSensor = mapping.sensors && mapping.sensors.includes(position)
