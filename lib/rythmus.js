...
module.exports = {
  ...
  get sensorIndexes () { return Object.values(configuration.sensors.mapping) },
  get sensors () {
    return Object.values(configuration.sensors.mapping).map(sensorPositon => pillars[sensorPositon])
  },
...
function getPillarsFromMapping (mapping) {
  ...
  Object.entries(mapping.nodes).forEach(([nodeName, node]) => {
    node.forEach((position, index) => {
      const isSensor = configuration.sensors.mapping && Object.values(configuration.sensors.mapping).includes(position)
